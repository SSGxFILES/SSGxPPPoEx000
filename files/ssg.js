    ...
    (function($) {
        $(document).ready(function() {
            ...
            if (BODY_INFO_BACKGROUND_IMAGE_ENABLE){
                $("#addCss").append('#logoContainer .card, #AccountInfo .card, #OwnerInfo .card, #PaymentBtn .card{background: '+BACKGROUND_COLOR+';background-image: url("/'+BODY_INFO_BACKGROUND_IMAGE+'");-webkit-background-size: cover !important;    -moz-background-size: cover!important;-o-background-size: cover!important;background-repeat: no-repeat !important; background-position: center center !important;background-attachment: fixed !important;border-radius: 10px;border: none;height: 100%;}');
            }else{
                $("#addCss").append('#logoContainer .card, #AccountInfo .card, #OwnerInfo .card, #PaymentBtn .card{background: '+BODY_INFO_BACKGROUND_COLOR+';border-radius: 10px;border: 1px solid '+BODY_INFO_BORDER_COLOR+';height: 100%;}');
            }
            ...
            $("body").prepend('<div id="WrapContainer" class="container"><div id="MyContainer"><div id="MainContainer" class="row col-md-12"><div id="AccountInfo" class="col-md-6"><div class="card"><div class="card-body"></div></div></div><div id="OwnerInfo" class="col-md-6"><div class="card"><div class="card-body"></div></div></div><div id="PaymentBtn" class="col-md-6" style="display:none"><div class="card"><div class="card-body" style="gap:10px"></div></div></div><div id="footer"></div></div></div></div>');
            GetStatus();
            AddLogo();
            BrandTitle();
            FooterHtmlCss();
    ...
    waitForElm("label:contains('Active'), label:contains('Expired')").then(()=>{
        ...
        if ((subscriptionStatus !== "") && (expirationDate !== "")) {
            ...
            if (subscriptionStatus == "Active"){
                $("#AccountInfo .card-body").append("<h5 class='ActiveStats'><i class='fa fa-wifi'></i><strong> "+ACTIVE_STATUS+" </strong><i class='fa fa-wifi'></i></h5>");
                $("#AccountInfo .card-body").append("<div><span class='statsLabel'>"+STATUS_LABEL +"</span>"+"<strong class='AStatus'> "+subscriptionStatus.toUpperCase() +" </strong></div>");
                $("#AccountInfo .card-body").append("<div><span class='expLabel'>"+EXPIRATION_LABEL +"</span>"+"<strong class='AEXP'> "+ExpformattedDate +" - "+ time12 +"</strong></div>");
                $("#AccountInfo .card-body").append("<div><span class='amountLabel'>"+AMOUNT_LABEL +"</span>"+"<strong class='AmountVal'> ₱ 000.00</strong></div>");
                $("#AccountInfo .card-body").append("<span class='activeTextInfo' style='margin-top: 10px;'>"+ACTIVE_TEXT_INFO+"</span>");  
                $("#OwnerInfo .card-body").append(MyInfo);
            }else{
                $("#AccountInfo .card-body").append("<h5 class='ExpStats'><i class='fa fa-exclamation-triangle'></i><strong> "+EXPIRED_STATUS+" </strong><i class='fa fa-exclamation-triangle'></i></h5>");
                $("#AccountInfo .card-body").append("<div><span class='statsLabel'>"+STATUS_LABEL +"</span>"+"<strong class='EStatus'> "+subscriptionStatus.toUpperCase() +" </strong></div>");
                $("#AccountInfo .card-body").append("<div><span class='expLabel'>"+EXPIRATION_LABEL +"</span>"+"<strong class='AEXP'> "+ExpformattedDate +" - "+ time12 +"</strong></div>");
                $("#AccountInfo .card-body").append("<div><span class='amountLabel'>"+AMOUNT_LABEL +"</span>"+"<strong class='AmountVal'> ₱ 000.00</strong></div>");
                $("#AccountInfo .card-body").append("<p class='expiredTextInfo' style='margin-top: 10px;'>"+EXPIRED_TEXT_INFO+"</p>");  
                $("#OwnerInfo .card-body").append(MyInfo);
            }
...
function AddLogo() {
    if (LOGOBANNER_ENABLE) {
        $("#MainContainer").prepend("<div id='logoContainer' class='col-md-6'><div class='card'><div class='card-body'><center><img id='logo' style='max-height: "+LOGOBANNER_HEIGHT+"px;' src='/"+LOGOBANNER_FILE_NAME+"'/></center></div></div></div>");
    }
}
...
function AddBank(){
    if ((ADD_GCASH_QR) || (ADD_PAYMAYA_QR) || (ADD_BPI_QR) || (ADD_SHOPEEPAY_QR) || (ADD_QRPH_QR) || (ADD_OTHER_QR)){
        if ($("#OnlineBankBTN").length === 0){
            $("#PaymentBtn .card-body").append("<span>MODE OF PAYMENT</span><div id='OnlineBankBTN' style='text-align: center;'></div>")
            $("#PaymentBtn").show();
