    ...
    (function($) {
        $(document).ready(function() {
            if (BACKGROUND_IMAGE_ENABLE){
                $("#addCss").append('#WrapContainer{background: '+BACKGROUND_COLOR+';background-image: url("/'+BACKGROUND_IMAGE+'");-webkit-background-size: cover !important;    -moz-background-size: cover!important;-o-background-size: cover!important;background-repeat: no-repeat !important; background-position: center center !important;background-attachment: fixed !important;}');
            }else{
                $("#addCss").append('#WrapContainer{background: '+BACKGROUND_COLOR+';}');
            }
            if (BODY_INFO_BACKGROUND_IMAGE_ENABLE){
                $("#addCss").append('#BodyContent{background: '+BACKGROUND_COLOR+';background-image: url("/'+BODY_INFO_BACKGROUND_IMAGE+'");-webkit-background-size: cover !important;    -moz-background-size: cover!important;-o-background-size: cover!important;background-repeat: no-repeat !important; background-position: center center !important;background-attachment: fixed !important;}');
            }else{
                $("#addCss").append('#BodyContent{background: '+BODY_INFO_BACKGROUND_COLOR+';}');
            }

            $("#addCss").append('#BrandTitleContainer{background: '+BRANDTITLE_BACKGROUND_COLOR+';border-radius: 0 0 10px 10px;border: 1px solid '+BRANDTITLE_BORDER_COLOR+'}#BodyContent{border-radius: 10px;border: 1px solid '+BODY_INFO_BORDER_COLOR+';}#footer{background: '+FOOTER_BACKGROUND_COLOR+';border-radius: 10px 10px 0 0;border: 1px solid '+FOOTER__BORDER_COLOR+';}.AStatus{color: '+STATUS_ACTIVE_COLOR+';}.EStatus{color: '+STATUS_EXPIRED_COLOR+';}.AEXP{color: '+EXPIRATION_COLOR+';}.AmountVal{color: '+AMOUNT_COLOR+';}.ActiveStats{color: '+ACTIVE_STATUS_COLOR+';}.statsLabel{color: '+STATUS_LABEL_COLOR+';}.expLabel{color: '+EXPIRATION_LABEL_COLOR+';}.amountLabel{color: '+AMOUNT_LABEL_COLOR+'}.activeTextInfo{color: '+ACTIVE_TEXT_INFO_COLOR+';}.ExpStats{color: '+EXPIRED_STATUS_COLOR+';}.expiredTextInfo{color: '+EXPIRED_TEXT_INFO_COLOR+';}.IFB{color: '+FB_NAME_COLOR+';}.IEMAIL{color: '+EMAIL_COLOR+';}.ICONTACT{color: '+CONTACT_NUMBER_COLOR+';}.ILOC{color: '+MYLOCATION_COLOR+';}#GcashBtn{background: '+GCASH_BAKGROUND_COLOR+';border: 1px solid '+GCASH_BORDER_COLOR+'}#PaymayaBtn{background: '+PAYMAYA_BAKGROUND_COLOR+';border: 1px solid '+PAYMAYA_BORDER_COLOR+'}#BPIBtn{background: '+BPI_BAKGROUND_COLOR+';border: 1px solid '+BPI_BORDER_COLOR+'}#SpayBtn{background: '+SHOPEEPAY_BAKGROUND_COLOR+';border: 1px solid '+SHOPEEPAY_BORDER_COLOR+'}#QRPHBtn{background: '+QRPH_BAKGROUND_COLOR+';border: 1px solid '+QRPH_BORDER_COLOR+'}#OtherBankBtn{background: '+OTHER_BAKGROUND_COLOR+';border: 1px solid '+OTHER_BORDER_COLOR+'}#OnlineBankQrContent p{color: '+SCREENSHOT_NOTE_COLOR+';}');
            $("body").prepend('<div id="WrapContainer"><div id="MyContainer" class="container-scroller"><div id="MainContainer" class="container"><div id="MainBody" class="mx-auto"><div id="BodyContent"><div id="InfoContainer"><div class="AccountInfo"></div></div></div></div><div id="footer"></div></div></div></div>');
            GetStatus();
            AddLogo();
            BrandTitle();
            FooterHtmlCss();
...
function GetStatus() {
    ...
    waitForElm("label:contains('Active'), label:contains('Expired')").then(()=>{
        ...
        if ((subscriptionStatus !== "") && (expirationDate !== "")) {
            ...
            var time12 = convertToAmPm(ExpTime);
            ...
            if (subscriptionStatus == "Active"){
                ...
                $(".AccountInfo").append("<p class='activeTextInfo' style='margin-top: 10px;'>"+ACTIVE_TEXT_INFO+"</p>");  
                $(".AccountInfo").after(MyInfo);
            }else{
                ...
                $(".AccountInfo").append("<div><span class='expLabel'>"+EXPIRATION_LABEL +"</span>"+"<strong class='AEXP'> "+ExpformattedDate +" - "+ time12 +"</strong></div>");
                $(".AccountInfo").append("<div><span class='amountLabel'>"+AMOUNT_LABEL +"</span>"+"<strong class='AmountVal'></strong></div>");
                $(".AccountInfo").append("<p class='expiredTextInfo' style='margin-top: 10px;'>"+EXPIRED_TEXT_INFO+"</p>");  
                $(".AccountInfo").after(MyInfo);
            }
            
            if (ENABLE_ONLINE_PAYMENT){
                $("#BodyContent").append($(".col-lg-6").parents().eq(2).attr("id","btn-container").addClass("card").attr("style", "margin-top: 10px;"));
                ...
                if (subscriptionStatus == "Expired" ){
                    $("#addCss").append(".group-btn .modal-backdrop{display: none;}.group-btn .tut{animation: blinkBtn 2s infinite;}");
                }
            }else{
...
function AddLogo() {
    if (LOGOBANNER_ENABLE) {
        $("#BodyContent").prepend("<div id='logoContainer' style='margin-bottom: 10px;'><center><img id='logo' style='max-height: "+LOGOBANNER_HEIGHT+"px;' src='/"+LOGOBANNER_FILE_NAME+"'/></center></div>");
    }
}
...
function AddBank(){
    if ((ADD_GCASH_QR) || (ADD_PAYMAYA_QR) || (ADD_BPI_QR) || (ADD_SHOPEEPAY_QR) || (ADD_QRPH_QR) || (ADD_OTHER_QR)){
        if ($("#OnlineBankBTN").length === 0){
            $("#BodyContent").append("<div id='OnlineBankBTN' style='margin-top: 20px; width: 100%; max-width: 700px; align-self: center;text-align: center;'><h6>MODE OF PAYMENT</h6><div class='group-btn2'></div></div>");
        ...
        if (ADD_GCASH_QR){
            $("#OnlineBankBTN .group-btn2").append("<button id='GcashBtn' class='btn' data-bs-toggle='modal' data-bs-target='#OnlineBankQrModal'><img class='img-btn' src='/files/gcash.svg' /></button>");
        }

        if (ADD_PAYMAYA_QR){
            $("#OnlineBankBTN .group-btn2").append("<button id='PaymayaBtn' class='btn' data-bs-toggle='modal' data-bs-target='#OnlineBankQrModal'><img class='img-btn' src='/files/maya.svg' /></button>");
        }

        if (ADD_BPI_QR){
            $("#OnlineBankBTN .group-btn2").append("<button id='BPIBtn' class='btn' data-bs-toggle='modal' data-bs-target='#OnlineBankQrModal'><img class='img-btn' src='/files/bpi.svg' /></button>");
        }

        if (ADD_SHOPEEPAY_QR){
            $("#OnlineBankBTN .group-btn2").append("<button id='SpayBtn' class='btn' data-bs-toggle='modal' data-bs-target='#OnlineBankQrModal'><img class='img-btn' src='/files/shopee.svg' /></button>");
        }

        if (ADD_QRPH_QR){
            $("#OnlineBankBTN .group-btn2").append("<button id='QRPHBtn' class='btn' data-bs-toggle='modal' data-bs-target='#OnlineBankQrModal'><img class='img-btn' src='/files/qrph.svg' /></button>");
        }

        if (ADD_OTHER_QR){
            $("#OnlineBankBTN .group-btn2").append("<button id='OtherBankBtn' class='btn' data-bs-toggle='modal' data-bs-target='#OnlineBankQrModal'><img class='img-btn' src='/"+BTN_LOGO_FILE+"' /></button>");
        }
        ...
        function showQrCode(buttonId) {
            ...
            contentDiv.append(`<img class='img-btn' style="margin-top: 5px" src='/${option.bankName}' />`);
...
function SetMargin(){
    ...
    function AutoMargin(){
        ...
        var TTLheight =  brandTitleHeight + FooterHeight;
        $('#WrapContainer').css('padding-top', (brandTitleHeight + 10) + 'px').css('padding-bottom', (FooterHeight + 10) + 'px').css('padding-left', '10px').css('padding-right', '10px');
        $('#MyContainer').css('height', 'calc(100vh - ' + (TTLheight + 20) + 'px').css('max-height',"100%").css('overflow',"auto");
    }
}
